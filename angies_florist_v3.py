...
""", unsafe_allow_html=True)
        ...
        if _parse_clicked:
            if raw_paste and raw_paste.strip():
                ...
                field_map = {
                    "parsed_customer_name":    result.sender_name,
                    "parsed_customer_contact": result.sender_phone,
                    "parsed_recipient_name":   result.recipient_name,
                    "parsed_recipient_contact":result.recipient_phone,
                    "parsed_delivery_address": result.delivery_address,
                    "parsed_landmark":         result.landmark,
                    "parsed_card_to":          result.card_to,
                    "parsed_card_body":        result.card_message,
                    "parsed_card_from":        result.card_from,
                    "parsed_notes":            result.special_notes,
                    "parsed_arrangement":      result.arrangement,
                    "parsed_order_type":       result.order_type or "Delivery",
                    "parsed_missing":          result.missing_fields,
                    "parsed_payment_method":   result.payment_method,
                    "parsed_payment_status":   result.payment_status,
                    "parsed_total":            result.total_amount,
                    "parsed_delivery_date":    result.delivery_date,
                    "parsed_delivery_time":    result.delivery_time,
                    "parsed_delivery_zone":    result.delivery_zone,
                }
                ...
                st.error("Nothing pasted — add the order form text above first.")

    with pcol_right:
        st.markdown("##### 📝 Parsed Summary")
        _has_parsed = any(
            st.session_state.get(f"parsed_{k}")
            for k in ["customer_name","recipient_name","delivery_address","arrangement"]
        )
        if not _has_parsed:
            st.caption("Paste an order on the left and click Auto-fill — extracted details will appear here for review.")
        else:
            _missing_set = set(st.session_state.get("parsed_missing", []))

            def _prow(icon, label, key):
                val = st.session_state.get(f"parsed_{key}", "")
                if not val:
                    return
                color = "#c0392b" if key in _missing_set else "#2d6a4f"
                st.markdown(
                    f"<div style='font-size:12px;margin-bottom:4px;'>"
                    f"<span style='color:#aaa;font-size:10px;text-transform:uppercase;"
                    f"letter-spacing:.08em;'>{icon} {label}</span><br>"
                    f"<span style='color:#1a1a1a;font-weight:600;'>{val}</span></div>",
                    unsafe_allow_html=True
                )

            _prow("👤", "Sender", "customer_name")
            _prow("📞", "Sender Phone", "customer_contact")
            _prow("🎯", "Recipient", "recipient_name")
            _prow("📞", "Recipient Phone", "recipient_contact")
            _prow("📍", "Address", "delivery_address")
            _prow("🗺️", "Zone", "delivery_zone")
            _prow("🏁", "Landmark", "landmark")
            _prow("📅", "Date", "delivery_date")
            _prow("🕐", "Time", "delivery_time")
            _prow("🌸", "Arrangement", "arrangement")
            _prow("💌", "Card To", "card_to")
            _prow("💬", "Message", "card_body")
            _prow("✍️", "From", "card_from")
            _prow("💳", "Payment", "payment_method")
            _prow("📋", "Order Details", "notes")

            if _missing_set:
                missing_labels = [f.replace("_", " ").title() for f in _missing_set]
                st.warning(f"⚠️ Missing: **{', '.join(missing_labels)}** — fill below manually.")
            else:
                st.success("✅ All fields extracted. Review above and fill the form below.")

            if st.button("🗑️ Clear", key="clear_paste", use_container_width=True):
                for _pk in ["parsed_customer_name","parsed_customer_contact",
                            "parsed_recipient_name","parsed_recipient_contact",
                            "parsed_delivery_address","parsed_landmark",
                            "parsed_card_to","parsed_card_body","parsed_card_from",
                            "parsed_notes","parsed_arrangement","parsed_order_type",
                            "parsed_missing","meta_paste_input","parsed_payment_method",
                            "parsed_payment_status","parsed_total","parsed_delivery_date",
                            "parsed_delivery_time","parsed_delivery_zone"]:
                    st.session_state.pop(_pk, None)
                st.rerun()

    # Helper to get parsed value or default
    def _pv(key, default=""):
        return st.session_state.get(f"parsed_{key}", default)

    # Missing field label helper
    _missing = set(st.session_state.get("parsed_missing", []))
    def _label(field_name, label):
        return f"{label} 🔴" if field_name in _missing else label
    ...
    with st.form("new_order_form", clear_on_submit=True):
        ...
        c1,c2 = st.columns(2)
        customer_name    = c1.text_input(_label("sender_name", "Customer Name *"),    value=vip_name if vip_name else _pv("customer_name"), placeholder="Juan dela Cruz")
        customer_contact = c2.text_input(_label("sender_phone", "Contact Number *"),   value=lookup_contact if lookup_contact else _pv("customer_contact"), placeholder="09XX-XXX-XXXX")

        st.markdown("##### 🎁 Recipient Details")
        rc1,rc2,rc3 = st.columns(3)
        recipient_name    = rc1.text_input(_label("recipient_name", "Recipient Name"),    value=_pv("recipient_name"), placeholder="Leave blank if same as buyer")
        ...
        with st.expander("💌 Message Card (Optional)"):
            ...
            msg_to   = mc1.text_input("To:",   value=_pv("card_to"),   placeholder="e.g., Maria")
...
""", unsafe_allow_html=True)
        ...
        if calc_balance > 0:
            # Auto-select payment method from parsed paste
            _pm = st.session_state.get("parsed_payment_method", "")
            _pm_idx = PAYMENT_METHODS_BALANCE.index(_pm) if _pm in PAYMENT_METHODS_BALANCE else 0
            balance_payment_method = st.selectbox("Balance Payment Method *", PAYMENT_METHODS_BALANCE, index=_pm_idx)

        st.markdown("##### 📅 Schedule & Location")
        c1,c2,c3 = st.columns(3)
        # Parse delivery date from paste if available
        _parsed_date = None
        if st.session_state.get("parsed_delivery_date"):
            try:
                from datetime import datetime as _dt
                _parsed_date = _dt.strptime(st.session_state["parsed_delivery_date"], "%B %d, %Y").date()
            except Exception:
                try:
                    _parsed_date = _dt.fromisoformat(st.session_state["parsed_delivery_date"]).date()
                except Exception:
                    _parsed_date = None
        _date_default = _parsed_date if _parsed_date and _parsed_date >= date.today() else date.today()
        # Parse delivery time from paste if available
        _parsed_time = None
        if st.session_state.get("parsed_delivery_time"):
            try:
                _traw = st.session_state["parsed_delivery_time"].strip().lower()
                _traw = _traw.replace("before ","").replace("after ","").strip()
                if ":" in _traw:
                    _parsed_time = datetime.strptime(_traw, "%I:%M%p").time() if "am" in _traw or "pm" in _traw else datetime.strptime(_traw, "%H:%M").time()
                elif "am" in _traw or "pm" in _traw:
                    _parsed_time = datetime.strptime(_traw, "%I%p").time()
            except Exception:
                _parsed_time = None
        _time_default = _parsed_time if _parsed_time else datetime.strptime("10:00","%H:%M").time()
        target_date = c1.date_input("Target Date *", min_value=date.today(), value=_date_default)
        target_time = c2.time_input("Target Time *", value=_time_default)
    ...
    if submitted:
        ...
        # Clear paste parser state
        for _pk in ["parsed_customer_name","parsed_customer_contact","parsed_recipient_name",
                    "parsed_recipient_contact","parsed_delivery_address","parsed_landmark",
                    "parsed_card_to","parsed_card_body","parsed_card_from","parsed_notes",
                    "parsed_arrangement","parsed_order_type","parsed_missing","meta_paste_input",
                    "parsed_payment_method","parsed_payment_status","parsed_total",
                    "parsed_delivery_date","parsed_delivery_time","parsed_delivery_zone"]:
            st.session_state.pop(_pk, None)
...
def page_rider_board():
    ...
    for idx, o in enumerate(filtered_orders):
        ...
        with st.expander(f"📦 {order_code} — {o['customer_name']} | Rider: {rider_name or '⏳ Unassigned'}"):
            ...
            # ── Waybill Print ──────────────────────────────────────────────
            st.markdown("#### 🧾 Waybill")
            _waybill_key = f"waybill_triggered_{order_code}"
            if st.button("🖨️ Print Waybill", key=f"waybill_rb_{order_code}", use_container_width=False):
                # Generate rider token if missing
                if not o.get("rider_token"):
                    _new_token = generate_order_token()
                    db.update_order(o["id"], {"rider_token": _new_token})
                    o["rider_token"] = _new_token
                st.session_state[_waybill_key] = True
            if st.session_state.get(_waybill_key):
                import base64
                import streamlit.components.v1 as components
                _wb_html = generate_waybill_html(adapt_order_for_waybill(o), o.get("rider_token"))
                _wb_b64  = base64.b64encode(_wb_html.encode("utf-8")).decode("utf-8")
                ...
                components.html(_wb_js, height=0, scrolling=False)
                st.session_state[_waybill_key] = False
                st.caption("✅ Waybill opened in new tab — print from there.")
            st.divider()
